...
public class NoteService {
    ...
    public NoteResponse updateNote(UUID noteId, UUID userId, NoteRequest request) {
        ...
        if (versionCount > 50) {
            ...
            List<NoteVersion> oldVersions = noteVersionRepository
                    .findByNoteIdOrderByEditedAtAsc(noteId, Pageable.ofSize((int) toDelete))
                    .getContent();
    ...
    public NoteResponse restoreVersion(
        ...
        Note note = noteRepository.findByIdAndEditableByUser(noteId, userId)
                .orElseThrow(() -> new IllegalArgumentException(
                        "Note not found or not editable"));

        NoteVersion targetVersion = noteVersionRepository.findByIdAndNoteId(versionId, noteId)
                .orElseThrow(() -> new IllegalArgumentException(
                        "Version not found"));
    ...
    public PageResponse<NoteVersionResponse> getNoteVersions(UUID noteId, UUID userId, Pageable pageable) {
        ...
        Page<NoteVersion> versions = noteVersionRepository.findByNoteIdOrderByEditedAtAsc(noteId, pageable);
